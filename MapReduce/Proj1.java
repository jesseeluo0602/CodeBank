...
public class Proj1{
     ...
     */
    /* DoublePair takes in two doubles, the first being hte frequency of the word appearing
       in the text, the second being the distance it is from the targeted word*/
    public static class Map1 extends Mapper<WritableComparable, Text, Text, DoublePair> {
        ...
        @Override
            ...
            throws IOException, InterruptedException {
                /*Add the index of the targeted word to an array list*/
                ...
                while (matcher.find()){
                    ...
                    if (word.equals(targetGram)){
                        targetIndex.add(targetindexes);
                    }
                    targetindexes=targetindexes+1;
                }
                /*run matcher again, if word is not in file it's distance is positive 
                infinity, otherwise take the minimum distance and write out a 
                text key and doublepair value*/
                while (matcher2.find()){
                    int distance=0;
                    double mindistance=Double.POSITIVE_INFINITY;
                    String word=matcher2.group().toLowerCase();
                    for (int i=0;i<targetIndex.size();i++){
                        distance=Math.abs(targetIndex.get(i)-currentindex);
    ...
    public static class Combine1 extends Reducer<Text, DoublePair, Text, DoublePair> {
        ...
        @Override
            public void reduce(Text key, Iterable<DoublePair> values,
                 ...
                 // YOUR CODE HERE
                /*Adds up the occurrences (frequency) and co-occurences(sum)*/
                double sum=0;
                double frequency=0;
                for (DoublePair dp: values){
                    frequency=frequency+dp.getDouble1();
                    sum=sum+dp.getDouble2();
                }
                context.write(key,new DoublePair(frequency,sum));
            }
    }
    ...
    public static class Reduce1 extends Reducer<Text, DoublePair, DoubleWritable, Text> {
        ...
        @Override
            public void reduce(Text key, Iterable<DoublePair> values,
                ...
                // YOUR CODE HERE
                /*Adds up the occurrences (frequency) and co-occurences(sum)
                writes out negative doublewritable in order for it to ber sorted
                correctly*/
                double score=0;
                double sum=0;
                double frequency=0;
                ...
                if (sum!=0){               
                    score=(sum*Math.pow(Math.log(sum),3))/frequency;
                }
    ...
    public static class Reduce2 extends Reducer<DoubleWritable, Text, DoubleWritable, Text> {
        ...
        @Override
            public void reduce(DoubleWritable key, Iterable<Text> values,
                    Context context) throws IOException, InterruptedException {
                 // YOUR CODE HERE
                /*Outputs and changes back the negative value to a positive value
                as it has already been previously sorted*/

                for (Text t:values){
                    double key2=key.get();
                    if (N_TO_OUTPUT>n){
                    context.write(new DoubleWritable(-key2),t);
